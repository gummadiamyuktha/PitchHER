...
function FormatSection({ format, stats, accent }) {
  ...
  return (
    <View style={[styles.formatSection, { borderLeftColor: accent, backgroundColor: '#FFF' }]}>
...
function BioSection({ player, accent }) {
  ...
  return (
    <View style={bioStyles.container}>
      <Text style={[styles.sectionHeader, { color: '#444' }]}>📖 PitchJourney</Text>
      {story ? (
...
function YearWiseSection({ yearWise, accent }) {
  ...
  return (
    <View style={styles.yearSection}>
      <Text style={[styles.sectionHeader, { color: '#444' }]}>📈 Year by Year</Text>
...
function HomeAwaySection({ homeAway, accent }) {
  ...
  return (
    <View style={haStyles.container}>
      <Text style={[styles.sectionHeader, { color: '#444' }]}>🏟️ Home vs Away</Text>
      <SlidingToggle options={formats} selected={currentFormat} onSelect={f => setActiveFormat(f)} accent={accent} />
...
function VSTeamSection({ vsTeams, accent }) {
  ...
  return (
    <View style={vsStyles.wrapper}>
      <Text style={[styles.sectionHeader, { color: '#444' }]}>🌍 VS Teams</Text>
...
function CoinTossCard({ batField, accent }) {
  ...
  return (
    <View style={ctStyles.wrapper}>
      <Text style={[styles.sectionHeader, { color: '#444' }]}>🪙 Coin Toss Analysis</Text>
    ...
    async function fetchPlayer() {
      try {
        if (playerParam) {
          ...
          // 🎤 Speak player name when card opens
          const speakName = PHONETIC_NAMES[playerData.name] || playerData.name;
          Speech.stop();
          setTimeout(() => {
            Speech.speak(
              `Let's dive into my stats!`,
